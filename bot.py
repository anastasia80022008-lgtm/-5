...
class UserStates(StatesGroup):
    ...
    waiting_chat = State() # Режим свободного общения
...
# --- ИИ ФУНКЦИЯ ---
async def ask_ai(prompt, photo_bytes=None, system_context="Ты ИИ-диетолог Вкусомер Плюс."):
    try:
        system_msg = system_context + " Если считаешь калории, в конце ВСЕГДА пиши строго: 'ИТОГО ККАЛ: [число]'. Будь дружелюбным и кратким."
        ...
        return f"🧘 Ошибка ИИ. Попробуй позже. ({e})"
...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("✨ Привет! Я Вкусомер Плюс. Давай рассчитаем твою норму калорий.\n\nВыбери свой пол:", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="Мужской"), KeyboardButton(text="Женский")]], resize_keyboard=True))
...
async def chat_start(message: types.Message, state: FSMContext):
    await message.answer("Я тебя слушаю! Задавай любой вопрос о питании, тренировках или просто поделись мыслями. 😊", reply_markup=chat_kb)
...
async def chat_process(message: types.Message, state: FSMContext):
    ...
    res = await ask_ai(message.text, system_context="Ты дружелюбный и экспертный диетолог. Отвечай на вопросы пользователя, поддерживай беседу.")
    await message.answer(res, reply_markup=chat_kb)
...
async def food_process(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"{ai_reply}\n\n📊 Итого дня: {total_today} / {data.get('daily_limit', 2000)} ккал", reply_markup=main_kb)
    await state.set_state(None)
...
async def progress(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📊 Вес: {w}кг → Цель: {tw}кг\n🔮 Результат через **~{days} дней**!")
...
async def psych(message: types.Message):
    ...
    await message.answer("Нажми на кнопку, если трудно.", reply_markup=kb)
