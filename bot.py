...
async def ask_dietologist(user_id, message_obj, system_type="default"):
    prompts = {
        "default": "Ты - Диетолог Вкусомер Плюс. Если пишут про еду - считай калории и пиши в конце 'ККАЛ: [число]'.",
        "chef": "Ты шеф-повар. Давай ОЧЕНЬ под��обные рецепты с граммами и шагами 1, 2, 3...",
...
async def proc_survey_finish(message: types.Message, state: FSMContext):
    try:
        ...
        db_commit("INSERT OR REPLACE INTO users (id, norma, total_today, water, last_date, weight, target) VALUES (?, ?, 0, 0, ?, ?, ?)",
                  (message.from_user.id, norma, str(datetime.now().date()), weight, data.get('target_w', weight)))
        ...
        await state.clear()
    except: await message.answer("Введи вес числом!")
